"""Page 2 — Career Recommendation Explorer.
...
st.title("🔍 Career Recommendation Explorer")
st.markdown(
    "Explore ranked career path recommendations tailored to your profile evidence. "
    "Every recommendation decomposes into machine learning dataset signals and ESCO taxonomy skill alignment."
)
...
with st.spinner("Calculating hybrid career recommendations..."):
...
for idx, rec in enumerate(recs):
    ...
    with st.expander(f"#{rank} — {role_title} (Score: {final_score:.3f})", expanded=(rank == 1)):
        ...
        with m_col1:
            st.metric("Composite Alignment", f"{final_score:.3f}")
        with m_col2:
        ...
        with ev_col1:
            ...
            if partial:
                st.markdown("##### ⚠️ Partial Skill Evidence")
                for p in partial:
...
st.markdown("---")
st.info(
    "ℹ️ **Score Interpretation**: Scores reflect relative model alignment and ESCO skill similarity on a [0, 1] scale. "
